 ...
 */
export function ensureSettlementVerifier(dataDir) {
  const walletFile = path.join(dataDir, "settlement-verifier.json");
  const keyFile = path.join(dataDir, "kascov-lab-key.hex");
  const faucetWallet = readJson(path.join(dataDir, "faucet-wallet.json"));
  ...
  if (!/^[0-9a-f]{64}$/i.test(wallet?.privateKey || "")) {
    ...
    wallet = {
      ...
      address: keypair.toAddress("testnet-10").toString(),
